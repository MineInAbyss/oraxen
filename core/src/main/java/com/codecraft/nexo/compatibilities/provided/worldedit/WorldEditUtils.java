...
public class WorldEditUtils {
    ...
    protected static class NexoBlockInputParser extends InputParser<BaseBlock> {

        public NexoBlockInputParser() {
        ...
        public BaseBlock parseFromInput(String input, ParserContext context) {
            ...
            if (!input.startsWith("nexo:") || input.endsWith(":")) return null;
            String id = input.split(":")[1].split("\\[")[0]; // Potential arguments
            boolean hasArguments = input.contains("[");
            if (id.equals(input) || !NexoBlocks.isCustomBlock(id)) return null;

            NoteBlockMechanic noteMechanic = NexoBlocks.getNoteBlockMechanic(id);
            BlockData blockData = NexoBlocks.getBlockData(id);
            if (blockData == null) return null;

            if (Settings.WORLDEDIT_STRINGBLOCKS.toBool() && NexoBlocks.isNexoStringBlock(id))
    ...
    protected static void pasteSchematic(Location loc, File schematic, Boolean replaceBlocks, Boolean shouldCopyBiomes, Boolean shouldCopyEntities) {
        ...
        try {
            ...
            try {
                ...
                NexoPlugin.get().getLogger().warning("Could not paste schematic for sapling-mechanic");
