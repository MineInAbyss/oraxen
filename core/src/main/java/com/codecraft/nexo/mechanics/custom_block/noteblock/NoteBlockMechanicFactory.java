...
import com.codecraft.nexo.mechanics.custom_block.noteblock.beacon.BeaconListener;
import com.codecraft.nexo.mechanics.custom_block.noteblock.beacon.BeaconTagDatapack;
import com.codecraft.nexo.mechanics.custom_block.noteblock.directional.DirectionalBlock;
import com.codecraft.nexo.mechanics.custom_block.noteblock.logstrip.LogStripListener;
...
public class NoteBlockMechanicFactory extends MechanicFactory {
    ...
    public static final NamespacedKey MINEABLE_PACKET_LISTENER = NamespacedKey.fromString("mineable_with_key", NexoPlugin.get());
    ...
    public NoteBlockMechanicFactory(ConfigurationSection section) {
        ...
        if (customSounds) MechanicsManager.registerListeners(NexoPlugin.get(), getMechanicID(), new NoteBlockSoundListener());

        BeaconTagDatapack.generateDatapack();

        // Physics-related stuff
        if (VersionUtil.isPaperServer())
            MechanicsManager.registerListeners(NexoPlugin.get(), getMechanicID(), new NoteBlockMechanicPaperListener());
        if (!VersionUtil.isPaperServer() || !NMSHandlers.isNoteblockUpdatesDisabled())
            MechanicsManager.registerListeners(NexoPlugin.get(), getMechanicID(), new NoteBlockMechanicPhysicsListener());
        if (!NMSHandlers.isNoteblockUpdatesDisabled()) {
            ...
            Logs.logWarning("Otherwise Nexo needs to listen to very taxing events, which also introduces some bugs");
            ...
            MechanicsManager.registerListeners(NexoPlugin.get(), getMechanicID(), new NoteBlockMechanicInstrumentListener());
    }
    ...
    public BlockState generateBlockState() {
        ...
        BlockState noteState = NexoPlugin.get().packGenerator().resourcePack().blockState(noteKey);
    ...
    public static void setBlockModel(Block block, String itemId) {
        NoteBlockMechanic mechanic = NexoBlocks.getNoteBlockMechanic(itemId);
