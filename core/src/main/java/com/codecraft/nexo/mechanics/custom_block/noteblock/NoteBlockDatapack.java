...
public class NoteBlockDatapack {
    ...
    private static final Path nexoDatapack = defaultWorld.getWorldFolder().toPath().resolve("datapacks/nexo_custom_blocks");

    public void generateDatapack() {
        nexoDatapack.resolve("data").toFile().mkdirs();
        writeMcMeta();
        removeFromMineableTag();

        Bukkit.getDatapackManager().getPacks().stream().filter(d -> d.getName().equals("file/nexo_custom_blocks")).findFirst().ifPresent(d -> d.setEnabled(true));
    }

    private void writeMcMeta() {
        try {
            File packMeta = nexoDatapack.resolve("pack.mcmeta").toFile();
    ...
    private void removeFromMineableTag() {
        try {
            File tagFile = nexoDatapack.resolve("data/minecraft/tags/block/mineable/axe.json").toFile();
