package com.codecraft.nexo.mechanics.storage;
...
public class StorageMechanic {
    ...
    public static final NamespacedKey STORAGE_KEY = new NamespacedKey(NexoPlugin.get(), "storage");
    public static final NamespacedKey PERSONAL_STORAGE_KEY = new NamespacedKey(NexoPlugin.get(), "personal_storage");
    ...
    public void dropStorageContent(Block block) {
        ...
        if (isShulker()) {
            ...
            ItemStack shulker = NexoItems.itemById(mechanic.getItemID()).build();
    ...
    public void dropStorageContent(FurnitureMechanic mechanic, ItemDisplay baseEntity) {
        ...
        ItemStack[] items = (frameStorages.containsKey(baseEntity) && gui != null)
                ? gui.getInventory().getContents() : pdc.getOrDefault(STORAGE_KEY, DataType.ITEM_STACK_ARRAY, new ItemStack[]{});
        if (isShulker()) {
            ItemStack defaultItem = NexoItems.itemById(mechanic.getItemID()).build();
