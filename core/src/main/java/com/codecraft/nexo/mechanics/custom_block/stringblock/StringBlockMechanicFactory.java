...
import com.codecraft.nexo.mechanics.custom_block.stringblock.sapling.SaplingListener;
import com.codecraft.nexo.mechanics.custom_block.stringblock.sapling.SaplingTask;
...
public class StringBlockMechanicFactory extends MechanicFactory {
    ...
    public StringBlockMechanicFactory(ConfigurationSection section) {
        ...
        MechanicsManager.registerListeners(NexoPlugin.get(), getMechanicID(), new StringBlockMechanicListener(), new SaplingListener());
        if (customSounds) MechanicsManager.registerListeners(NexoPlugin.get(), getMechanicID(), new StringBlockSoundListener());

        // Physics-related stuff
        if (VersionUtil.isPaperServer())
            MechanicsManager.registerListeners(NexoPlugin.get(), getMechanicID(), new StringBlockMechanicPaperListener());
        if (!VersionUtil.isPaperServer() || !NMSHandlers.isTripwireUpdatesDisabled())
            MechanicsManager.registerListeners(NexoPlugin.get(), getMechanicID(), new StringBlockMechanicPhysicsListener());
    ...
    public BlockState generateBlockState() {
        ...
        BlockState stringState = NexoPlugin.get().packGenerator().resourcePack().blockState(stringKey);
    ...
    public void registerSaplingMechanic() {
        ...
        saplingTask.runTaskTimer(NexoPlugin.get(), 0, saplingGrowthCheckDelay);
        sapling = true;
    }
}
