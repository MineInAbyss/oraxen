...
import static com.codecraft.nexo.utils.BlockHelpers.isLoaded;
import static com.codecraft.nexo.utils.blocksounds.BlockSounds.*;
...
public class FurnitureSoundListener implements Listener {
    ...
    public void onBreakingStone(final BlockBreakEvent event) {
        ...
        if (NexoBlocks.isNexoStringBlock(block) || block.getType() == Material.TRIPWIRE && mechanicBelow != null && mechanicBelow.isTall()) return;
        if (block.getBlockData().getSoundGroup().getBreakSound() != Sound.BLOCK_STONE_BREAK) return;
        if (NexoFurniture.isFurniture(block.getLocation()) && block.getType() == Material.BARRIER || block.isEmpty()) return;
    ...
    public void onHitStone(final BlockDamageEvent event) {
        ...
        BukkitTask task = Bukkit.getScheduler().runTaskTimer(NexoPlugin.get(), () ->
    ...
    public void onStepFall(final GenericGameEvent event) {
        ...
        FurnitureMechanic mechanic = NexoFurniture.getFurnitureMechanic(blockStandingOn.getLocation());
    ...
    public void onPlacingFurniture(final NexoFurniturePlaceEvent event) {
    ...
    @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
    public void onBreakingFurniture(final NexoFurnitureBreakEvent event) {
