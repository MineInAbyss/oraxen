...
import com.codecraft.nexo.font.FontEvents;
import com.codecraft.nexo.nms.GlyphHandlers;
import com.codecraft.nexo.pack.PackObfuscator;
import com.codecraft.nexo.pack.server.PackServerType;
...
public enum Settings {
    ...
    COMMENT("Pack.generation.comment", """
            ...
            belongs to the owner of the Nexo
    ...
    POLYMATH_SERVER("Pack.server.polymath.server", "atlas.mineinabyss.com"),
    POLYMATH_SECRET("Pack.server.polymath.secret", "mineinabyss"),
    ...
    // Inventory
    NEXO_INV_LAYOUT("nexo_inventory.menu_layout", Map.of(
    ...
    NEXO_INV_ROWS("nexo_inventory.menu_rows", 6),
    NEXO_INV_SIZE("nexo_inventory.menu_size",  45),
    NEXO_INV_TITLE("nexo_inventory.main_menu_title", "<shift:-18><glyph:menu_items><shift:-193>"),
    NEXO_INV_NEXT_ICON("nexo_inventory.next_page_icon"),
    NEXO_INV_PREVIOUS_ICON("nexo_inventory.previous_page_icon"),
    NEXO_INV_EXIT("nexo_inventory.exit_icon");
    ...
    public Object getValue() {
        return NexoPlugin.get().configsManager().getSettings().get(path);
    }
    public void setValue(Object value) { setValue(value, true); }
    public void setValue(Object value, boolean save) {
        YamlConfiguration settingFile = NexoPlugin.get().configsManager().getSettings();
        settingFile.set(path, value);
        try {
            if (save) settingFile.save(NexoPlugin.get().getDataFolder().toPath().resolve("settings.yml").toFile());
        } catch (Exception e) {
    ...
    public List<String> toStringList() {
        return NexoPlugin.get().configsManager().getSettings().getStringList(path);
    }

    public ConfigurationSection toConfigSection() {
        return NexoPlugin.get().configsManager().getSettings().getConfigurationSection(path);
    }

    public static YamlConfiguration validateSettings() {
        File settingsFile = NexoPlugin.get().getDataFolder().toPath().resolve("settings.yml").toFile();
        YamlConfiguration settings = settingsFile.exists() ? NexoYaml.loadConfiguration(settingsFile) : new YamlConfiguration();
