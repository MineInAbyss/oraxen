...
import com.codecraft.nexo.items.ItemTemplate;
...
public class ConfigsManager {
    ...
    private YamlConfiguration extractDefault(String source) {
        ...
        try {
            return NexoYaml.loadConfiguration(inputStreamReader);
        } finally {
            try {
    ...
    public void validatesConfig() {
        ResourcesManager resourcesManager = NexoPlugin.get().resourceManager();
    ...
    private YamlConfiguration validate(ResourcesManager resourcesManager, String configName, YamlConfiguration defaultConfiguration) {
        ...
        YamlConfiguration configuration = NexoYaml.loadConfiguration(configurationFile);
    ...
    public Collection<Glyph> parseGlyphConfigs() {
        ...
        for (File file : glyphFiles) {
            ...
            YamlConfiguration configuration = NexoYaml.loadConfiguration(file);
    ...
    private void validatePackSection(String itemId, ConfigurationSection packSection) {
        ...
        if (!Key.parseable(model)) {
            Logs.logWarning("Found invalid model in NexoItem <blue>" + itemId + "</blue>: <aqua>" + model);
        ...
        for (String texture : packSection.getStringList("textures")) {
            if (!Key.parseable(texture)) {
                Logs.logWarning("Found invalid texture in NexoItem <blue>" + itemId + "</blue>: <aqua>" + texture);
    ...
    public Map<String, ItemBuilder> parseItemConfig(File itemFile, ItemBuilder errorItem) {
        YamlConfiguration config = NexoYaml.loadConfiguration(itemFile);
    ...
    private List<File> getItemFiles() {
        ...
        return FileUtils.listFiles(itemsFolder, new String[]{"yml"}, true).stream().filter(NexoYaml::isValidYaml).sorted().toList();
    }

    private List<File> getGlyphFiles() {
        if (glyphsFolder == null || !glyphsFolder.exists()) return new ArrayList<>();
        return FileUtils.listFiles(glyphsFolder, new String[]{"yml"}, true).stream().filter(NexoYaml::isValidYaml).sorted().toList();
    }
}
