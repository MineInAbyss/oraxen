...
import com.codecraft.nexo.api.events.resourcepack.NexoPackUploadEvent;
import com.codecraft.nexo.api.events.resourcepack.NexoPostPackGenerateEvent;
import com.codecraft.nexo.api.events.resourcepack.NexoPrePackGenerateEvent;
...
public class PackGenerator {
    ...
    public static Path externalPacks = NexoPlugin.get().packPath().resolve("external_packs");
    static final NexoPackReader reader = new NexoPackReader();
    static final NexoPackWriter writer = new NexoPackWriter();

    private static final Path assetsFolder = NexoPlugin.get().packPath().resolve("assets");
    ...
    public static void stopPackGeneration() {
        Optional.ofNullable(NexoPlugin.get().packGenerator()).ifPresent(packGenerator -> {
    ...
    public void generatePack() {
        stopPackGeneration();
        EventUtils.callEvent(new NexoPrePackGenerateEvent(resourcePack));
        ...
        packGenFuture = CompletableFuture.allOf(futures.toArray(new CompletableFuture[]{})).thenRunAsync(() -> {
            ...
            try {
                NexoPack.mergePack(resourcePack, reader.readFromDirectory(NexoPlugin.get().packPath().toFile()));
            } catch (Exception e) {
            ...
            try {
                resourcePack = Bukkit.getScheduler().callSyncMethod(NexoPlugin.get(), () -> {
                    NexoPostPackGenerateEvent event = new NexoPostPackGenerateEvent(resourcePack);
            ...
            if (resourcePack.packMeta() == null) resourcePack.packMeta(NMSHandlers.getHandler().mcmetaVersion(), "Nexo's default pack.");

            File packZip = NexoPlugin.get().packPath().resolve("pack.zip").toFile();
            ...
            NexoPlugin.get().packServer().uploadPack().thenRun(() -> {
                Bukkit.getScheduler().callSyncMethod(NexoPlugin.get(), () -> EventUtils.callEvent(new NexoPackUploadEvent(builtPack, NexoPlugin.get().packServer().packUrl())));
    ...
    private void addGlyphFiles() {
        ...
        for (Glyph glyph : NexoPlugin.get().fontManager().glyphs()) {
    ...
    private void addSoundFile() {
        for (SoundRegistry customSoundRegistry : NexoPlugin.get().soundManager().customSoundRegistries()) {
    ...
    private void importRequiredPack() {
        ...
        try {
            NexoPack.mergePack(resourcePack, requiredPack.get().isDirectory()
    ...
    private void importDefaultPack() {
        ...
        try {
            ...
            Logs.logError("Failed to read Nexo's RequiredPack...");
            if (!Settings.DEBUG.toBool()) Logs.logError(e.getMessage());
            else e.printStackTrace();
        }
    ...
    private void importExternalPacks() {
        ...
        for (File file : externalPacks) {
            ...
            if (file.isDirectory() || file.getName().endsWith(".zip")) {
                ...
                try {
                    NexoPack.mergePack(resourcePack, reader.readFile(file));
                } catch (Exception e) {
    ...
    private void importModelEnginePack() {
       ...
       try {
           NexoPack.mergePack(resourcePack, reader.readFromZipFile(megPack));
       } catch (Exception e) {
    ...
    public NexoPackReader getPackReader() {
        return reader;
    }

    public NexoPackWriter getPackWriter() {
        return writer;
    }
}
