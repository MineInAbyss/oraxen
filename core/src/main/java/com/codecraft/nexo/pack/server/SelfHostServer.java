...
public class SelfHostServer implements NexoPackServer {
    ...
    private final ResourcePackRequestHandler handler = (request, exchange) -> {
        Writable packData = NexoPlugin.get().packGenerator().builtPack().data();
    ...
    public String packUrl() {
        String hash = NexoPlugin.get().packGenerator().builtPack().hash();
        int serverPort = Settings.SELFHOST_PACK_SERVER_PORT.toInt(8082);
    ...
    public void sendPack(Player player) {
        NexoPlugin.get().packGenerator().packGenFuture.thenRun(() -> {
            String hash = NexoPlugin.get().packGenerator().builtPack().hash();
            byte[] hashArray = NexoPackServer.hashArray(hash);
            String url = packUrl();
    ...
    public CompletableFuture<Void> uploadPack() {
        String hashPart = "/" + NexoPlugin.get().packGenerator().builtPack().hash() + ".zip";
