...
public class PolymathServer implements NexoPackServer {
    ...
    public PolymathServer() {
        String address = Settings.POLYMATH_SERVER.toString("atlas.mineinabyss.com");
    ...
    public boolean isPackUploaded() {
        return NexoPlugin.get().packGenerator().packGenFuture.isDone() && uploadFuture != null && uploadFuture.isDone();
    }

    @Override
    public CompletableFuture<Void> uploadPack() {
        if (!Objects.equals(hash, NexoPlugin.get().packGenerator().builtPack().hash())) {
        ...
        if (uploadFuture == null) uploadFuture = CompletableFuture.runAsync(() -> {
            try(CloseableHttpClient httpClient = HttpClients.createDefault()) {
                ...
                HttpEntity httpEntity = MultipartEntityBuilder.create()
                        ...
                        .addPart("pack", new ByteArrayBody(NexoPlugin.get().packGenerator().builtPack().data().toByteArray(), "pack"))
                        .build();
                ...
                if (jsonOutput.has("url") && jsonOutput.has("sha1")) {
                    ...
                    NexoPackServer.allPackUUIDs.add(packUUID);
    ...
    public void sendPack(Player player) {
        if (!NexoPlugin.get().packGenerator().packGenFuture.isDone()) return;
        if (uploadFuture == null || !uploadFuture.isDone()) return;

        NexoPackServer.allPackUUIDs.add(packUUID);
        Set<UUID> oldPackUUIDs = new HashSet<>(NexoPackServer.allPackUUIDs);
