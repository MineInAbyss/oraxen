...
public class RecipesManager {
    ...
    public static void load(JavaPlugin plugin) {
        if (Settings.RESET_RECIPES.toBool()) {
            ...
            while (recipeIterator.hasNext()) {
                ...
                if (recipeID.getNamespace().equals("nexo")) {
        ...
        File recipesFolder = new File(NexoPlugin.get().getDataFolder(), "recipes");
        if (!recipesFolder.exists()) {
            recipesFolder.mkdirs();
            if (Settings.GENERATE_DEFAULT_CONFIGS.toBool())
                NexoPlugin.get().resourceManager().extractConfigsInFolder("recipes", "yml");
            else try {
    ...
    private static void registerConfigRecipes(File configFile) {
        YamlConfiguration config = NexoYaml.loadConfiguration(configFile);
