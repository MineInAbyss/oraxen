...
public class ClickAction {
    ...
    private static ClickAction from(final LinkedHashMap<String, Object> config) {
        ...
        final List<Action<Player>> actions =
                NexoPlugin.get().clickActionManager().parse(Player.class, (List<String>) config.getOrDefault("actions", Collections.emptyList()));
    ...
    public static ClickAction from(final ConfigurationSection config) {
        ...
        final List<Action<Player>> actions = NexoPlugin.get().clickActionManager().parse(Player.class, config.getStringList("actions"));
    ...
    public void performActions(final Player player) {
        NexoPlugin.get().clickActionManager().run(player, actions, false);
    }
