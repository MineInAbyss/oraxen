...
import com.codecraft.nexo.pack.DefaultResourcePackExtractor;
...
public class TrimsCustomArmor {
    ...
    public static final Key datapackKey = Key.key("minecraft:file/nexo_custom_armor");
    private static final File customArmorDatapack = defaultWorld.getWorldFolder().toPath().resolve("datapacks/nexo_custom_armor").toFile();
    ...
    private static void writeCustomTrimPatterns(LinkedHashSet<String> armorPrefixes) {
        for (String armorPrefix : armorPrefixes) {
            ...
            JsonObject description = new JsonObject();
            description.addProperty("translate", "trim_pattern.nexo." + armorPrefix);
            trimPattern.add("description", description);
            trimPattern.addProperty("asset_id", "nexo:" + armorPrefix);
    ...
    private void writeTrimAtlas(ResourcePack resourcePack, LinkedHashSet<String> armorPrefixes) {
        ...
        if (trimsAtlas != null) {
            ...
            for (AtlasSource source : trimsAtlas.sources()) {
                ...
                for (String armorPrefix : armorPrefixes) {
                    textures.add(Key.key("nexo:trims/models/armor/" + armorPrefix));
                    textures.add(Key.key("nexo:trims/models/armor/" + armorPrefix + "_leggings"));
                }
    ...
    private void copyArmorLayerTextures(ResourcePack resourcePack) throws IOException {
        ...
        String nexoTrimPath = "nexo:trims/models/armor/";
        ...
        new LinkedHashSet<>(resourcePack.textures()).forEach(armorTexture -> {
            ...
            if (armorTexture.key().asString().endsWith("_armor_layer_1.png"))
                resourcePack.texture(Key.key(nexoTrimPath + armorPrefix + ".png"), armorTexture.data());
            else if (armorTexture.key().asString().endsWith("_armor_layer_2.png"))
                resourcePack.texture(Key.key(nexoTrimPath + armorPrefix + "_leggings.png"), armorTexture.data());
        });
    ...
    private void checkNexoArmorItems() {
        ...
        for (ItemBuilder itemBuilder : NexoItems.items()) {
            String itemID = NexoItems.idByItem(itemBuilder);
            ...
            if (!itemBuilder.itemFlags().contains(ItemFlag.HIDE_ARMOR_TRIM)) {
            ...
            if (!itemBuilder.hasTrimPattern()) {
                ...
                TrimPattern trimPattern = Registry.TRIM_PATTERN.get(NamespacedKey.fromString("nexo:" + armorPrefix));
                if (trimPattern == null) {
