...
import static com.codecraft.nexo.utils.AdventureUtils.*;
...
public class FontEvents implements Listener {
    ...
    public void registerChatHandlers() {
        if (paperChatHandler != null)
            Bukkit.getPluginManager().registerEvents(paperChatHandler, NexoPlugin.get());
        if (legacyPaperChatHandler != null)
            Bukkit.getPluginManager().registerEvents(legacyPaperChatHandler, NexoPlugin.get());
        if (spigotChatHandler != null)
            Bukkit.getPluginManager().registerEvents(spigotChatHandler, NexoPlugin.get());
    }
    ...
    public void onBookGlyph(final PlayerEditBookEvent event) {
        ...
        for (String page : meta.getPages()) {
            ...
            if (i == 0) continue;
            for (Character character : manager.reverseMap().keySet()) {
                if (!page.contains(String.valueOf(character))) continue;

                Glyph glyph = manager.glyphFromName(manager.reverseMap().get(character));
    ...
    public void onBookGlyph(final PlayerInteractEvent event) {
        ...
        for (String page : meta.getPages()) {
            ...
            for (Map.Entry<String, Glyph> entry : manager.glyphByPlaceholderMap().entrySet()) {
        ...
        NexoPlugin.get().audience().player(player).openBook(book);
    }
    ...
    public void onPlayerRename(final InventoryClickEvent event) {
        ...
        if (displayName != null) {
            ...
            for (Character character : manager.reverseMap().keySet()) {
                ...
                Glyph glyph = manager.glyphFromName(manager.reverseMap().get(character));
                if (!glyph.hasPermission(player)) {
                    Glyph required = manager.glyphFromName("required");
        ...
        if (((displayName == null || displayName.isEmpty()) && NexoItems.exists(inputItem)) || strippedDownInputDisplay.equals(displayName)) {
    ...
    public class SpigotChatHandler implements Listener {
        ...
        private String format(String string, @Nullable Player player) {
            ...
            if (player != null) for (Character character : manager.reverseMap().keySet()) {
